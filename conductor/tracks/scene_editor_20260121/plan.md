...
- [ ] Task: 实现变换编辑器 (Transform Editor)
    - [ ] 为选中节点提供 Position, Rotation, Scale 的 DragFloat 输入框
    - [ ] 验证父节点变换修改后，子节点矩阵自动同步更新
- [ ] Task: 实现材质编辑器 (Material Editor)
    - [ ] 自动识别选中节点是否包含网格/材质
    - [ ] 提供 PBR 参数（Albedo, Roughness 等）的实时滑块
